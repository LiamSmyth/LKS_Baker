"""Optional named filter dispatch with per-filter defaults."""
...
from . import blur_cpu, normalize_cpu
...
def run_filter(name: str, image: np.ndarray, **overrides: Any) -> np.ndarray:
    """Run a registered filter by name; ``overrides`` replace defaults."""
    ...
    if name == "blur":
        ...
        return blur_cpu.filter(image, island_id, sigma, sample_mask=sample_mask)
    if name == "normalize_signed":
        valid = params.pop("valid")
        return normalize_cpu.filter_signed(image, valid, **params)
    if name == "normalize_positive":
        valid = params.pop("valid")
        return normalize_cpu.filter_positive(image, valid, **params)
    raise KeyError(f"unknown image filter: {name!r}")
