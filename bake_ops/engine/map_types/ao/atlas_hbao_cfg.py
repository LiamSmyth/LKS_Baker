"""Static config for the ``atlas_hbao`` bake method."""
...
@dataclass
class AtlasHbaoConfig:
    """Engine defaults until per-method RNA is wired."""

    pass


def config_from_entry(entry: LKS_PG_BakeMapEntry) -> AtlasHbaoConfig:
    """Build config from map entry (defaults only for now)."""
    _ = entry
    return AtlasHbaoConfig()
