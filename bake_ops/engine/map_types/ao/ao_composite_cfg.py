"""Static config for the ``ao_composite`` bake method."""
...
@dataclass
class AoCompositeConfig:
    """Engine defaults until per-method RNA is wired."""

    pass


def config_from_entry(entry: LKS_PG_BakeMapEntry) -> AoCompositeConfig:
    """Build config from map entry (defaults only for now)."""
    _ = entry
    return AoCompositeConfig()
